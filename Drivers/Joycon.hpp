...
public:
	...
	void send_subcommand(int command, int subcommand, uint8_t *data, int len) {
		...
		uint8_t rumble_base[9] = { (unsigned char)((++global_count) & 0xF), 0x00, 0x01, 0x40, 0x40, 0x00, 0x01, 0x40, 0x40 };
	...
	void setGyroOffsets() {
		float thresh = 0.1;
		if (std::abs(this->gyro.roll) > thresh || std::abs(this->gyro.pitch) > thresh || std::abs(this->gyro.yaw) > thresh) {
			return;
		}
